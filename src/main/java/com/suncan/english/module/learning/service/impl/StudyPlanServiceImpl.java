package com.suncan.english.module.learning.service.impl;
...
import com.suncan.english.shared.enums.GoalTypeEnum;
...
public class StudyPlanServiceImpl implements StudyPlanService {
    ...
    @Override
    public StudyPlanPageVO listPlans(Long userId, StudyPlanListQueryDTO queryDTO) {
        validatePlanListQuery(queryDTO);

        long current = normalizeCurrent(queryDTO == null ? null : queryDTO.getCurrent());
        long size = normalizeSize(queryDTO == null ? null : queryDTO.getSize());
        long offset = (current - 1L) * size;

        LambdaQueryWrapper<StudyPlan> countWrapper = buildPlanListWrapper(userId, queryDTO, false);
        Long totalValue = studyPlanMapper.selectCount(countWrapper);
        long total = totalValue == null ? 0L : totalValue;

        List<StudyPlanVO> records = new ArrayList<>();
        if (total > 0) {
            LambdaQueryWrapper<StudyPlan> pageWrapper = buildPlanListWrapper(userId, queryDTO, true)
                    .last("limit " + offset + "," + size);
            List<StudyPlan> planList = studyPlanMapper.selectList(pageWrapper);
            for (StudyPlan plan : planList) {
                records.add(toPlanVO(plan));
            }
        }

        StudyPlanPageVO pageVO = new StudyPlanPageVO();
     ...
     */
    private void validatePlanListQuery(StudyPlanListQueryDTO queryDTO) {
        if (queryDTO == null || queryDTO.getStatus() == null) {
            return;
        }
        Integer status = queryDTO.getStatus();
        if (!StudyPlanStatusEnum.RUNNING.getCode().equals(status)
                && !StudyPlanStatusEnum.FINISHED.getCode().equals(status)) {
            throw new BusinessException("status only supports 1/2");
        }
    }
     ...
     * 校验任务状态是否合法。
     */
    private LambdaQueryWrapper<StudyPlan> buildPlanListWrapper(Long userId, StudyPlanListQueryDTO queryDTO, boolean withOrder) {
        LambdaQueryWrapper<StudyPlan> wrapper = new LambdaQueryWrapper<StudyPlan>()
                .eq(StudyPlan::getUserId, userId);
        if (queryDTO != null && queryDTO.getStatus() != null) {
            wrapper.eq(StudyPlan::getStatus, queryDTO.getStatus());
        }
        if (withOrder) {
            wrapper.orderByDesc(StudyPlan::getId);
        }
        return wrapper;
    }
